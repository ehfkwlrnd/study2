...
귀류법을 통해 증명하도록 하자.
	...
	3)	임의의 i < k < j인 ck는 반드시 ci 또는 cj와 부호가 같다.
		일관성을 잃지 않고 ck가 ci와 부호가 다르다고 가정하자.
		xy < 0 이면 |x + y| = ||x| - |y|| 임을 활용하자.
		특히 ci <= ck <= cj 이므로
		|ci + cj| = |cj| - |ci|
		|ci + ck| = |ck| - |ci| 이다.
		따라서 |ci + ck| < |ci + cj| 임을 알 수 있다. 
따라서 |ci + cj|가 최소라면 |i - j| = 1 이다. 

그래서 위와 같은 알고리즘은 정렬하는데 O(nlog(n)) 이고, 비교하는데 O(n) 이므로
O(nlog(n))의 시간복잡도를 갖는다. 
*/ 
